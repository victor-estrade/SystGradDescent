...
def run_estimation(args, i_cv):
    ...
    iter_results = [run_estimation_iter(model, result_row, i, test_config, valid_generator, test_generator, calib_rescale)
...
def run_estimation_iter(model, result_row, i_iter, config, valid_generator, test_generator, calib_rescale):
    logger = logging.getLogger()
    logger.info('-'*45)
