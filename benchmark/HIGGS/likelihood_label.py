...
from problem.higgs.nll import LabelNLL
...
def run(args, i_cv):
    ...
    result_table = [run_iter(i_cv, i, test_config, valid_generator, test_generator, directory) for i, test_config in enumerate(config.iter_test_config())]
...
# TODO : Remove
    ...
    X_test, y_test, w_test = test_generator.generate(*config.CALIBRATED, n_samples=config.N_TESTING_SAMPLES)
