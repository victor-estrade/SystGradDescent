...
# Command line :
# python -m benchmark.HIGGS.explore.minuit_bfgs
...
def parse_args(main_description="Explore NLL shape"):
    ...
    parser.add_argument("--tolerance", type=float,
                        default=10, help="tolerance value for Minuit migrad and simplex minimization")
...
def run_cv_iter(args, i_cv, i_iter, config, root_directory):
    ...
    values.update(config.TRUE.to_dict(prefix="TRUE_"))
...
def get_generators(args, i_cv=0):
    seed = SEED + i_cv * 5
...
def minuit_migrad_to_values_dict(minimizer):
    updates = dict(tes = minimizer.values[0]
